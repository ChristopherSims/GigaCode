...
class QueryCache:
    ...
    def _make_key(
        self, buffer_id: str, query: str, top_k: int, mode: str
    ...
    def _find_semantically_similar(
        self, query: str, buffer_id: str, top_k: int, mode: str, query_embedding: np.ndarray
        ...
        if self._embedder is not None and self._similarity_threshold < 1.0:
            try:
                ...
                if query_emb.shape[0] > 0:
                    ...
                    similar_key = self._find_semantically_similar(
                        query, buffer_id, top_k, mode, query_embedding
                    )
    ...
    def stats(self) -> dict[str, int | float]:
        ...
        semantic_rate = (
            ((self._semantic_hits / total_requests) * 100) if total_requests > 0 else 0.0
        )
