...
def _chunk_with_tree_sitter(
    text: str, language_hint: str | None, file_hint: str = ""
) -> list[CodeChunk] | None:
    ...
    for node in _walk(root):
        if node.type in definition_types:
            ...
            if node.type == "preprocessing_directive":
                ...
                # Ruby lambda/proc
                chunk_type = "lambda"
            elif "function" in node.type or node.type in (
                "arrow_function",
                "function_item",
                "function_declaration",
            ):
                ...
                # Default fallback
                if "class" in node.type or node.type in (
                    "struct_item",
                    "struct_specifier",
                    "object_declaration",
                ):
...
def chunk_file(
    path: str | Path, language_hint: str | None = None, sliding_window_size: int = 30
) -> list[CodeChunk]:
