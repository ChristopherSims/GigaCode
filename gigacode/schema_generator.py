...
def generate_schema_from_method(method_name: str, method: Callable) -> dict[str, Any]:
    ...
    for param_name, param in sig.parameters.items():
        ...
        if param.annotation != inspect.Parameter.empty:
            ...
            # Check annotation
            if param.annotation == int or "int" in annotation_str:
                param_type = "integer"
            elif param.annotation == float or "float" in annotation_str:
                param_type = "number"
            elif param.annotation == bool or "bool" in annotation_str:
                param_type = "boolean"
            elif param.annotation == list or "list" in annotation_str:
                param_type = "array"
            elif param.annotation == dict or "dict" in annotation_str:
                param_type = "object"
...
def validate_schemas_against_code(
    tool_class: Any,
    hardcoded_schemas: dict[str, dict[str, Any]]
) -> dict[str, list[str]]:
    ...
    for method_name, hardcoded in hardcoded_schemas.items():
        ...
        gen_required = set(
            generated_schema.get("input_schema", {}).get("required") or []
        )
        hard_required = set(
            hardcoded.get("input_schema", {}).get("required") or []
        )
