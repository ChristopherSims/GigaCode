...
class DependencyGraph:
        ...
        for ch in self.chunks:
            for imp in ch.imports or []:
                ...
                for other_ch in self.chunks:
                    ...
                    if (
                        imp.replace(".", "/") in other_ch.file
                        or imp.split(".")[-1] in Path(other_ch.file).stem
                    ):
                        self._reverse_deps.setdefault(other_ch.file, []).append(ch.file)

    def trace_call_chain(
        self, from_symbol: str, to_symbol: str, max_depth: int = 10
    ) -> CallChainResult:
        """Find call chain from from_symbol to to_symbol using BFS."""
        visited: set[str] = set()
        queue: list[tuple[str, list[dict[str, Any]]]] = [
            (from_symbol, [{"symbol": from_symbol, "file": "", "line": 0}])
        ]
    ...
    def get_dependencies(self, file: str, direction: str = "outgoing") -> list[str]:
        ...
        if direction == "outgoing":
            ...
            return list(set(self._dependencies.get(file, []) + self._reverse_deps.get(file, [])))
    ...
    def export_graph(self, format: str = "json") -> dict[str, Any]:
        ...
        if format == "json":
            return {
                "nodes": list({ch.file for ch in self.chunks}),
                "edges": [
