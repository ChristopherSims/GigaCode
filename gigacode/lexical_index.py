...
class LexicalIndex:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        Includes document tokens and computed statistics (IDF, TF, doc lengths).
        The index does NOT need to be dirty (already rebuilt) to serialize.
        """
        if self._dirty:
            self._rebuild()
        return {
            ...
            "avgdl": self._avgdl,
            "idf": self._idf,
            "tf": {str(doc_id): tf_map for doc_id, tf_map in self._tf.items()},
        }

    @classmethod
    def from_dict(cls, data: dict[str, Any]) -> LexicalIndex:
        """Reconstruct a LexicalIndex from serialized data."""
        index = cls(k1=data.get("k1", 1.5), b=data.get("b", 0.75))
        index._docs = data.get("docs", {})
        index._doc_len = {int(k): v for k, v in data.get("doc_len", {}).items()}
        index._avgdl = data.get("avgdl", 1.0)
        index._idf = data.get("idf", {})
        index._tf = {int(k): v for k, v in data.get("tf", {}).items()}
        index._dirty = False
        return index
