...
class IndexManager:
    ...
    def _get_index(self, buffer_id: str) -> GpuIndex | None:
        ...
        try:
            ...
            json_logger.debug(
                operation="load_index", details={"buffer_id": buffer_id, "from": "disk"}
            )

            return index
        except (OSError, json.JSONDecodeError, TypeError, ValueError) as exc:
            json_logger.warning(
                operation="load_index", message=f"Failed to load index for {buffer_id}: {exc}"
            )
            return None
    ...
    def _get_lexical_index(self, buffer_id: str) -> LexicalIndex | None:
        ...
        try:
            ...
            json_logger.debug(
                operation="load_lexical_index", details={"buffer_id": buffer_id, "from": "disk"}
            )
    ...
    def _load_chunks(self, buffer_id: str) -> list[CodeChunk] | None:
        ...
        try:
            ...
            json_logger.warning(
                operation="load_chunks", message=f"Failed to load chunks for {buffer_id}: {exc}"
            )
            return None
    ...
    def _check_gpu_memory(self) -> dict[str, Any]:
        ...
        try:
            ...
            json_logger.warning(
                operation="check_gpu_memory", message=f"GPU memory check failed: {exc}"
            )
    ...
    def _evict_gpu_indices(self, buffer_id: str) -> None:
        ...
        try:
            ...
            json_logger.debug(operation="evict_gpu_index", details={"buffer_id": buffer_id})
    ...
    def _save_buffer_state(
        ...
        try:
            ...
            json_logger.error(
                operation="save_buffer_state", message=f"Failed to save buffer state: {exc}"
            )
            return {"status": "error", "message": str(exc)}
    ...
    def load_buffer_state(
        ...
        if embeddings_path.exists():
            try:
                ...
                json_logger.warning(
                    operation="load_embeddings", message=f"Failed to load embeddings: {exc}"
                )
    ...
    def _rebuild_files(
        ...
        if embeddings is None or chunks is None:
            ...
            if embeddings is None or chunks is None:
                return {"status": "error", "message": "Could not load buffer state for rebuild"}
                ...
                try:
                    ...
                    for file_path in files:
                        if file_path in chunks_by_file:
                            ...
                            changed, removed, kept = (
                                self._incremental_manager._chunk_tracker.detect_changes(
    ...
    def close(self) -> None:
        ...
        json_logger.info(operation="close", message="IndexManager closed and resources released")
