...
class IntentClassifier:
    ...
    KEYWORDS_BY_CATEGORY = {
        IntentCategory.FEATURE_ADDITION: [
            "add", "implement", "create", "new feature", "support", "enable",
            "introduce", "allow", "build", "develop"
        ],
        IntentCategory.BUG_FIX: [
            "fix", "bug", "error", "crash", "break", "issue", "fail", "broken",
            "not working", "problem", "wrong", "incorrect"
        ],
        IntentCategory.REFACTORING: [
            "refactor", "cleanup", "reorganize", "simplify", "improve", "restructure",
            "rename", "extract", "consolidate", "optimize", "performance"
        ],
        IntentCategory.DOCUMENTATION: [
            "document", "docstring", "comment", "example", "explain", "readme",
            "guide", "tutorial", "api", "help"
        ],
        IntentCategory.TESTING: [
            "test", "unittest", "coverage", "mock", "verify", "assert", "validate"
        ],
        IntentCategory.OPTIMIZATION: [
            "optimize", "fast", "speed", "performance", "efficient", "memory",
            "cache", "reduce latency", "throughput"
        ],
        IntentCategory.CLEANUP: [
            "cleanup", "remove", "delete", "dead code", "unused", "deprecate",
            "retire", "decommission"
        ],
    }
...
class ActionPlanner:
    ...
    def plan(
        ...
        with timer("plan_actions"):
            ...
            if category == IntentCategory.BUG_FIX:
                actions, total_tokens = self._plan_bug_fix(
                    intent, buffer, budget
                )
            elif category == IntentCategory.FEATURE_ADDITION:
                actions, total_tokens = self._plan_feature(
                    intent, buffer, budget
                )
            elif category == IntentCategory.REFACTORING:
                actions, total_tokens = self._plan_refactoring(
                    intent, buffer, budget
                )
            elif category == IntentCategory.OPTIMIZATION:
                actions, total_tokens = self._plan_optimization(
                    intent, buffer, budget
                )
            else:
                actions, total_tokens = self._plan_generic(
                    intent, buffer, budget
                )
            ...
            total_duration = sum(
                getattr(a, 'estimated_duration_ms', 0) for a in actions
            )
    ...
    def _extract_error_keywords(intent: str) -> str:
        ...
        error_words = [w for w in words if len(w) > 3 and w not in [
            "error", "bug", "fix", "what", "does", "break"
        ]]
