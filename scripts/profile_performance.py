...
class PerformanceProfiler:
    ...
    def profile_faiss_search(self):
        ...
        try:
            ...
            with self.profile_operation(f"Generate {num_vectors} random embeddings (dim {dimension})"):
    ...
    def print_summary(self):
        ...
        print("\nTop 5 Slowest Operations:")
        for i, result in enumerate(sorted(self.results, key=lambda r: r.duration_ms, reverse=True)[:5], 1):
            print(f"  {i}. {result.operation}: {result.duration_ms:.2f}ms")
        
        # Top 5 memory consumers
        print("\nTop 5 Memory Consumers:")
        for i, result in enumerate(sorted(self.results, key=lambda r: r.memory_mb_delta, reverse=True)[:5], 1):
