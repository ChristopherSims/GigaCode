...
from gigacode.state_manager import FileLocker, StateManager, TransactionLog
...
class TestStateManager:
    ...
    def test_save_and_load_registry(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Set buffer info
            manager.set_buffer_info(
                "buf-1", {"path": "/code", "chunk_count": 100, "dirty_files": {}}
            )
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            def update_buffer(buffer_id, count):
                ...
                for i in range(count):
                    mgr.set_buffer_info(
                        buffer_id, {"path": f"/code{i}", "chunk_count": i, "dirty_files": {}}
                    )
...
class TestStateManagerEdgeCases:
    ...
    def test_multiple_cache_types_invalidation(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            manager.invalidate_cache_for_buffer("buf-1", ["index", "lexical"])
