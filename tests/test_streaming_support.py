...
class TestStreamingFileReader:
    ...
    def test_large_file_multiple_chunks(self):
        ...
        try:
            ...
            for _i, (content, start_line, end_line) in enumerate(chunks):
        ...
        try:
            ...
            def handler(content, start_line, end_line):
                chunks_processed.append(
                    {"start": start_line, "end": end_line, "size": len(content)}
                )
...
"""
        ...
        try:
            ...
            chunker = StreamingChunker(max_chunk_bytes=10 * 1024, language="javascript")
            chunks = []
