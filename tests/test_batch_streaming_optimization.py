...
from gigacode.embedder_optimizer import OptimizedEmbedder, wrap_embedder_with_optimization
...
def test_streaming_support():
    ...
    try:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            print(f"✅ BufferManager created with streaming support")
            
            # Verify method exists
            assert hasattr(buffer_mgr, 'embed_file_with_streaming')
            print(f"✅ embed_file_with_streaming method available")
...
def test_large_file_detection():
    ...
    try:
        ...
        print(f"✅ Small file (10MB) correctly detected as no-stream needed")
        
        # Test large file
        large_size = 100 * 1024 * 1024  # 100MB
        assert supports_streaming(large_size, threshold_mb=50)
        print(f"✅ Large file (100MB) correctly detected as streaming needed")
        
        # Test custom threshold
        assert supports_streaming(large_size, threshold_mb=200) == False
        print(f"✅ Custom threshold (200MB) working correctly")
